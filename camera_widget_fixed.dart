...
/// Convertit YUV_420_888 en NV21 pour Android
Uint8List _convertYUV420ToNV21(CameraImage image) {
  ...
  final int yPixelStride = yPlane.pixelStride;

  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      final int yIndex = y * yRowStride + x * yPixelStride;
      nv21[y * width + x] = yPlane.bytes[yIndex];
    }
  }

  // Plans UV (chrominance) - entrelacés en NV21 (VUVU...)
  final Plane uPlane = image.planes[1];
  final Plane vPlane = image.planes[2];
  final int uvRowStride = uPlane.bytesPerRow;
  final int uvPixelStride = uPlane.pixelStride;
  ...
  for (int y = 0; y < uvHeight; y++) {
    for (int x = 0; x < uvWidth; x++) {
      ...
      final int nv21Index = ySize + y * width + x * 2;
...
// Configuration et debug
  static const bool _kShowDebugButton =
      false; // Mettre à false pour cacher le bouton debug
...
// La caméra et la galerie afficheront automatiquement les dialogs de permission
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    // Fix: vérification séparée pour éviter NPE
    ...
    try {
      _cameraController!.startImageStream((CameraImage image) async {
        ...
        try {
          ...
          if (frameCount % 30 == 0) {
            // Log toutes les 10 frames traitées
            debugPrint('⚠️ Erreur détection frame: $e');
          }
        } finally {
          // Fix: suppression du délai artificiel pour meilleure performance
  ...
  InputImageRotation _getImageRotation() {
    ...
    if (Platform.isIOS) {
      ...
      debugPrint("🔄 Rotation iOS: sensor=$sensorOrientation° → $rotation");
      return rotation;
    }

    // Android: compensation selon l'orientation du device
    ...
    debugPrint(
        "🔄 Rotation Android: sensor=$sensorOrientation°, device=$deviceOrientation, final=$rotationDegrees° → $rotation");
  ...
  Future<List<Face>> _detectFacesFromCameraImage(CameraImage image) async {
    try {
      ...
      if (Platform.isAndroid) {
        // Fix: Gestion correcte des formats Android
        if (image.planes.length == 1) {
          // Format NV21 natif (rare)
          bytes = image.planes[0].bytes;
          format = InputImageFormat.nv21;
          debugPrint("📱 Android: Format NV21 natif détecté");
        } else {
          // Format YUV_420_888 (standard) → conversion nécessaire
          bytes = _convertYUV420ToNV21(image);
          format = InputImageFormat.nv21;
          if (DateTime.now().millisecondsSinceEpoch % 1000 < 50) {
            // Log occasionnel
            debugPrint(
                "📱 Android: Conversion YUV_420_888 → NV21 (${image.planes.length} planes)");
          }
        }
      } else {
  ...
  void _updateFeedback(List<Face> faces) {
    ...
    // Fix: Throttle pour éviter trop de setState
    ...
    setState(() {
      ...
      if (allChecksPass) {
        ...
        final horizontalOffset =
            (faceCenterX - screenCenterX).abs() / imageWidth;
      ...
      if (allChecksPass) {
        ...
        final verticalOffset =
            (faceCenterY - screenCenterY).abs() / imageHeight;
  ...
  Future<void> _switchCamera() async {
    ...
    try {
      ...
      // Fix: Délai avant recalibration pour stabilisation
  ...
  Future<void> _takePhoto() async {
    ...
    try {
      ...
      // Fix: Timeout sans callback, laisse throw naturellement
